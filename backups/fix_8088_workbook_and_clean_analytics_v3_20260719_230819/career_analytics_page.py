...
def render_career_analytics_page(master_path: str | Path) -> None:
    ...
    st.caption(f"Authoritative source: {analytics['metadata']['workbook_name']} · {analytics['metadata']['workbook_path']}")
    validation=analytics["validation"]
    if validation["validation_status"]=="PASSED": st.success("Workbook validation passed")
    else:
    ...
    for warning in validation.get("warnings",[]): st.info(warning)
    s=analytics["summary"]

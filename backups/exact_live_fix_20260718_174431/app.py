    ...
    components.html(
        ...
        (function () {
            ...
            function initReplay() {
                ...
                // QUICKLAUNCH_LITERAL_START
                function bindQuickLaunchTransition(buttonId, transitionUrl, destinationUrl, durationMs) {
                    const button = doc.getElementById(buttonId);
                    if (!button || button.dataset.quickLaunchBound === "1") return;
                    button.dataset.quickLaunchBound = "1";
                    button.addEventListener("click", function(event) {
                        event.preventDefault(); event.stopPropagation();
                        const existing = doc.getElementById("journeyQuickLaunchFrame");
                        if (existing) existing.remove();
                        const frame = doc.createElement("iframe");
                        frame.id = "journeyQuickLaunchFrame"; frame.src = transitionUrl;
                        frame.style.position = "fixed"; frame.style.inset = "0";
                        frame.style.width = "100vw"; frame.style.height = "100dvh";
                        frame.style.border = "0"; frame.style.zIndex = "2147483647"; frame.style.background = "#000";
                        doc.body.appendChild(frame);
                        win.setTimeout(function(){ win.location.href = destinationUrl; }, durationMs);
                    }, true);
                }
                bindQuickLaunchTransition("journeyBronxButton","http://100.70.235.51:8020/experiences/baseball/?debug=1","http://100.70.235.51:8000/",6200);
                bindQuickLaunchTransition("journeyShowcaseButton","http://100.70.235.51:8020/experiences/family/?debug=1","http://100.70.235.51:9000/",7000);
                // QUICKLAUNCH_LITERAL_END
...
def render_logo_factory_page() -> None:
    ...
    st.markdown(
        """
        <div style="display:flex;justify-content:flex-end;margin:0 0 14px 0;">
          <a href="http://100.70.235.51:8090" target="_blank" style="display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:10px 18px;border:1px solid rgba(168,85,247,.72);border-radius:14px;background:linear-gradient(135deg,rgba(60,32,88,.96),rgba(28,18,46,.96));color:#fff;font-weight:800;letter-spacing:.04em;text-decoration:none;box-shadow:0 0 18px rgba(168,85,247,.18);">🧪 Logo Lab</a>
        </div>
        """,
        unsafe_allow_html=True,
    )
